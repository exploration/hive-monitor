...
#SYNC_GROUPS="fm_facultyfeedbackadmin,hr_admin,hr_user,print_lists_admin,portico_director,portico_curriculum,portico_rd,portico_banker,portico_shopper,portico_staff,portico_triage"
WORKDIR="/tmp"
...
dsexport "${WORKDIR}/groups" "${GOOGLE_LDAP}" dsRecTypeStandard:Groups 
wc -l "${WORKDIR}/groups"

# import google groups
echo "Synchronizing groups..."
dsimport "${WORKDIR}/groups" "/LDAPv3/127.0.0.1" O --remotehost "${EXPLO}" --remoteusername "${USER}" --remotepassword "${PASS}" --username "${USER}" --password "${PASS}" --outputfile "${WORKDIR}/groups.log"
